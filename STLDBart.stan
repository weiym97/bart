...
transformed parameters {
  ...
  alpha = exp(mu_pr[4] + sigma[4] * alpha_pr);
...
model {
  ...
  for (j in 1:N) {
    ...
    if (explosion[j,k] ==0){
      omega = omega * (1 + vwin[j] * pumps[j,k] / (P * (1 + alpha[j] * (k-1))));
    }
    else{
      omega = omega * (1 - vloss[j] * (1 - pumps[j,k] * inv(P)) / (1 + alpha[j] * (k-1)));
    }
...
generated quantities {
  ...
  real<lower=0> mu_alpha = exp(mu_pr[4]);
