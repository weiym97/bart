...
model {
  ...
  for (j in 1:N) {
    ...
    for (k in 1:Tsubj[j]) {
      ...
      temp_0 = C * lambda[j] * exp(beta[j] * RPE) * p_burst - C * log1m(p_burst) - B * inv(lambda[j]);
      temp_1 = B * lambda[j] * exp(beta[j] * RPE) * p_burst - 2 * A * inv(lambda[j]) - B * log1m(p_burst);
      temp_2 = A * lambda[j] * exp(beta[j] * RPE) * p_burst- A * log1m(p_burst);
