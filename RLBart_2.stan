...
transformed parameters {
  ...
  gamma = mu_pr[3] + sigma[3] * gamma_pr;
...
model {
  ...
  for (j in 1:N) {
    ...
    if (explosion[j,k] ==0){
      Q = r_accu[pumps[j,k] + 1] - alpha[j] * (r_accu[pumps[j,k] + 1] - Q) + gamma[j];
    }
    else{
      if (r_accu[pumps[j,k]] - Q<0)
          Q = Q + lambda[j] * (r_accu[pumps[j,k]] - Q);
    }
...
generated quantities {
  ...
  real mu_alpha = mu_pr[2];
  real mu_gamma = mu_pr[3];
