  ...
  for (j in 1:N) {
    ...
    for (k in 1:Tsubj[j]) {
      ...
      if (explosion[j,k] ==0){
          ...
          omega = omega + alpha[j] * (pumps[j,k] * inv(P) - omega);
        }
        else{
          Loss_aver = (1 - theta[j]) * Loss_aver;
          omega = omega - beta[j] * (1 + omega - pumps[j,k] * inv(P));
...
generated quantities {
  ...
  real<lower=0> mu_theta = exp(mu_pr[4]);
  real<lower=0,upper=1> mu_lambda = Phi_approx(mu_pr[5]);
